...
from job_search_mcp.paths import DEFAULT_JOB_PROFILE, SCHEMAS_DIR

PROFIL_SCHEMA = SCHEMAS_DIR / "job-profil.schema.json"
STANDARD_PROFIL = DEFAULT_JOB_PROFILE
...
def validiere_profil_dict(
    rohdaten: dict[str, Any], schema_pfad: Path = PROFIL_SCHEMA
) -> dict[str, Any]:
...
def profil_aus_dict(rohdaten: dict[str, Any]) -> JobProfil:
    ...
    if (
        ...
        raise ValueError(
            f"gehalt_min darf nicht groesser als gehalt_max sein: {gehalt_min} > {gehalt_max}"
        )
    return JobProfil(
