...
Jede `*.yaml`-Datei in diesem Verzeichnis ist ein Portal-Profil (`job_portal.PortalProfil`).
...
- `stepstone.yaml` wurde per expliziter Nutzerfreigabe (2026-08-12) auf
  ...
  `mvps/stepstone_calib.py` zugangsdatenfrei live kalibriert. Der
...
(`unterricht/job_browser.py`, `mvps/camoufox_browser.py`). Der
`browser_session.py`-Manager speichert `storage_state` außerhalb des Projekts;
...
`job_feeds.py` kapselt Transport und Anbieterformate hinter einem gemeinsamen
...
`profiles/portal-katalog.yaml` ist der fachliche Katalog. Er ist bewusst vom
