...
from unterricht.job_models import (
    JobAngebot,
    JobMatch,
    JobProfil,
    normalisiere_menge,
...
def _skill_abgedeckt(skills: frozenset[str], gewuenscht: str) -> bool:
    ...
    """
    return any(
        skill == gewuenscht or skill.startswith(gewuenscht + " ")
        for skill in skills
    )
...
def _bewerte_einzel(profil: JobProfil, angebot: JobAngebot) -> JobMatch:
    ...
    if angebot.sprachen and profil.sprachen and not (angebot.sprachen & profil.sprachen):
        passt = False
    ...
    score = round(pflicht_anteil * 40 + wunsch_anteil * 20 + ort_score + modell_score + gehalt_score)
    score = min(100, score)
    if passt:
