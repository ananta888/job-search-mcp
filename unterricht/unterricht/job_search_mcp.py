...
from unterricht.browser_session import (
    BrowserSessionFehler,
    BrowserSessionManager,
    BrowserUseTreiber,
)
from unterricht.credential_store import CredentialStore
from unterricht.job_agent_mcp import angebot_zu_dict, match_zu_dict, profil_zu_dict
from unterricht.job_feeds import suche_feed
...
from unterricht.job_match import bewerte_angebote as bewerte_kern
from unterricht.job_models import JobAngebot, JobProfil
from unterricht.job_portal import PortalProfil, lade_portal_katalog
...
def _state_dir() -> Path:
    ...
    return basis / "unterricht-job-search"
...
def portal_suche(
    ...
    portal = _portal(portal_id)
    _gate_check(portal)
    profil = lade_profil_modell(_projekt_pfad(profil_pfad))
    if query is None:
...
def lade_profil(profil_pfad: str = "profiles/job-suchprofil.json") -> dict[str, object]:
    ...
    return profil_zu_dict(lade_profil_modell(_projekt_pfad(profil_pfad)))
