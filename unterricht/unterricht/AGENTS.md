...
These instructions apply to the complete `unterricht/` subtree.

They specialize the repository-level `AGENTS.md` for a small, Docker-free teaching project.
Higher-priority system and user instructions always take precedence. General safety, quality,
security, and repository rules from the parent file remain valid.
...
The `todos/` directory is the planning and progress system for this teaching project. Todo JSON
